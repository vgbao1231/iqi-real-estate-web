...
export function OtherTab({
  ...
  const TimelinePreview = () => {
    ...
    const renderLayout = () => {
      const count = progressImages.length;

      // 2 tấm: grid 2x1
      if (count === 2) {
        return (
          <div className="grid grid-cols-2 gap-4 h-[400px]">
            {progressImages.map((image: string, index: number) => (
            ...
            <div className="grid grid-cols-1 gap-4 h-[300px]">
              <div className="flex justify-center">
                <div className="w-full max-w-md">
                  <ImageCard image={progressImages[0]} index={0} />
                </div>
              </div>
            </div>
            {/* Dòng 2: 3 ảnh */}
      ...
      if (count >= 7) {
        ...
        while (currentIndex + 5 <= count) {
          const group = progressImages.slice(currentIndex, currentIndex + 5);
        ...
        const remaining = progressImages.length - currentIndex;
        const leftoverImages = progressImages.slice(currentIndex);
