...
const Timeline = forwardRef<HTMLElement, { data: any }>(({ data }, ref) => {
  ...
  const renderLayout = () => {
    ...
    console.log(data.progressImages, count);
    ...
    if (count >= 7) {
      ...
      while (currentIndex + 5 <= count) {
        ...
        result.push(
          <div key={`group-${currentIndex}`} className="space-y-4 mb-6">
            {/* Dòng 1: 2 ảnh */}
            <div className="grid grid-cols-2 gap-4 h-[300px]">
      ...
      return <div className="space-y-8">{result}</div>;
    }
  };
...
const ImageCard = ({ image, index }: any) => (
  <FadeIn
    delay={index * 0.15}
    className="group relative overflow-hidden rounded-2xl bg-white shadow-xl hover:shadow-2xl cursor-pointer h-full"
  >
